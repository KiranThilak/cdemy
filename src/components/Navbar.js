...
const Navbar = () => {
    ...
    return (
        <header className={`bg-white sticky top-0 left-0 right-0 z-50 ${isScrolled ? 'py-2' : 'py-4'} transition-all duration-300`} style={{ boxShadow: isScrolled ? '0 2px 4px -1px rgba(0, 0, 0, 0.1)' : '0 4px 6px -1px rgba(0, 0, 0, 0.05)' }}>
            <div className="flex justify-between items-center w-92 mx-auto px-4">
                ...
                <div className="flex items-center mt-2 mr-2 ">
                    <NavLink to="/contactpage" className="relative middle none center uppercase text-sm px-4 py-2 rounded-full bg-gradient-to-r from-indigo-500 to-indigo-600  text-white shadow-lg hover:shadow-xl hover:shadow-indigo-600/40 focus:shadow-outline focus:outline-none active:top-[2px] active:shadow-none disabled:pointer-events-none disabled:opacity-50" onClick={() => setIsOpen(!isOpen)}>
                        <span className="absolute top-0 right-0 bottom-0 left-0 w-full h-full rounded-full bg-indigo-900 opacity-75"></span>
                        <span className="relative">Contact</span>
                    </NavLink>
                    <span className="text-3xl cursor-pointer md:hidden" onClick={() => setIsOpen(!isOpen)}>{isOpen ? '✖' : '☰'}</span>
                </div>
